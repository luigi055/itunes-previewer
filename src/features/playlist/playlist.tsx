...
import { SortOptions,TitleWithSort,TrackElement } from "./playlist-collaborators";
import {
  PlayListWrapper
  ...
  return (
    <>
      ...
      <PlayListWrapper>
        ...
        {searchSong.sortedTracks.map((trackData, index) => <TrackElement key={`${trackData.collectionName}${trackData.trackName}${trackData.trackId}`} trackData={trackData} trackPath={getURLPath(index)}/>)}
      </PlayListWrapper>
    </>
  );
};
